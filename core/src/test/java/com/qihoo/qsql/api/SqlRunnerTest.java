...
public class SqlRunnerTest {
    ...
    public void testSingleQueryForMySql() {
        ...
        Assert.assertEquals(buildDynamicSqlRunner().sql(sql).getClass(),
                JdbcPipeline.class);
    }

    @Test
    public void execSingleQueryForMySql() {
        String sql = "SELECT dep_id FROM edu_manage.department";

        AbstractPipeline sql1 = buildDynamicSqlRunner().sql(sql);
    ...
    public void testSingleQueryForHive() {
        String sql = "SELECT 1";
        Assert.assertEquals(buildSparkSqlRunner().sql(sql).getClass(),
                SparkPipeline.class);
    }
    ...
    public void testMixQueryBetweenMySqlAndHive() {
        String sql = "SELECT a.dep_id, b.stu_id"
                + " FROM edu_manage.department AS a, action_required" +
                ".homework_content AS b"
    ...
    public void testMixQueryBetweenMySqlAndElasticsearch() {
        String sql = "SELECT a.dep_id, b.stu_id"
                + " FROM edu_manage.department AS a, student_profile.student " +
                "AS b"
                + " WHERE a.dep_id = b.stu_id";
        Assert.assertEquals(buildDynamicSqlRunner().sql(sql).getClass(),
                SparkPipeline.class);
    ...
    public void testMixQueryBetweenElasticsearchAndHive() {
        String sql = "SELECT a.stu_id, b.stu_id"
                + " FROM student_profile.student AS a, action_required" +
                ".homework_content AS b"
    ...
    public void testMixQueryBetweenMySqlAndHiveWithFlink() {
        String sql = "SELECT a.dep_id, b.stu_id"
                ...
                ".homework_content AS b"
                + " WHERE a.dep_id = b.stu_id";
        Assert.assertEquals(buildFlinkSqlRunner().sql(sql).getClass(),
                FlinkPipeline.class);
    }
